 ...
 *
 */
class Query extends \PHPhoenix\DB\Query
{
     ...
     *
     * @param \PHPhoenix\DB\PDO\Connection $db Database connection
     * @param string $type Query type. Available types: select, update, insert, delete, count
     * @return \PHPhoenix\DB\PDO\Query
     * @see \PHPhoenix\DB\Query::__construct()
     */
	 ...
	 * If you pass an \PHPhoenix\DB\Expression object the value will be inserted into the query unescaped
	 *
	 * @param mixed $field     Field to be escaped or an \PHPhoenix\DB\Expression object
	 *                         if the field must not be escaped
	 * @return string  Escaped field representation
	 * @see \PHPhoenix\DB\Expression
	 */
	public function escape_field($field, $prepend_table = true)
	{
		if (is_object($field) && $field instanceof \PHPhoenix\DB\Expression)
		{
			return $field->value;
		}
		$field = explode('.', $field);
	 ...
	 * If you pass an \PHPhoenix\DB\Expression object the value will be inserted into the query unescaped
	 * @param mixed $val     Value to be escaped or an \PHPhoenix\DB\Expression object
	...
	{
		if ($val instanceof \PHPhoenix\DB\Expression)
		{
		...
		if ($val instanceof \PHPhoenix\DB\Query)
		{
	 ...
	 *
	 * @param \PHPhoenix\DB\Query $query Query builder for the subquery
	 * @param array  &$params Reference to parameters array
     ...
     *
     * @param string|\PHPhoenix\DB\Expression|\PHPhoenix\DB\Query|array $table Table representation
     * @param array &$params Reference to parameters array
     * @throws \Exception
	...
	public function escape_table($table, &$params)
	{
		...
		if ($table instanceof \PHPhoenix\DB\Query)
			return "{$this->subquery($table, $params)} AS {$this->quote($alias)}";

		if ($table instanceof \PHPhoenix\DB\Expression)
		...
		else
		{
			...
			if (count($this->_union) > 0 && ($this->_type == 'select'))
			{
				...
				foreach ($this->_union as $union)
				{
					...
					if ($union[0] instanceof \PHPhoenix\DB\Query)
					{
						$query .= $this->subquery($union[0], $params);
					}
					elseif ($union[0] instanceof \PHPhoenix\DB\Expression)
					{
						$query .= "({$union[0]->value}) ";
					}
					else
					{
						throw new \Exception("You can only use query builder instances or \$phoenix->db->expr() for unions");
					}
				}
			}
