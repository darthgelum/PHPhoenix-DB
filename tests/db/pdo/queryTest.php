...
class PDO_Query_Test extends PHPUnit_Framework_TestCase
{
	...
	protected function setUp()
	{
		...
		$this->object = new \PHPhoenix\DB\PDO\Query($stub, 'select');
	}
	...
	public function testEscape_field()
	{
		...
		$this->assertEquals('test', $this->object->escape_field(new \PHPhoenix\DB\Expression('test')));
	...
	{
		$params = array();
		$this->assertEquals('test', $this->object->escape_value(new \PHPhoenix\DB\Expression('test'), $params));
	...
	public function testQuerySelect1()
	{
		$query = $this->object
			...
			->order_by('id', 'desc')
			->group_by('id')
			->having('j', '<', new \PHPhoenix\DB\Expression('korova'))
	...
	public function testQueryExpressionParams()
	{
		$query = $this->object
			->type('select')
			->table('fairies')
			->where('id', 'MATCHES', new \PHPhoenix\DB\Expression("AGAINST ?", array('phoenix')))
			->query();

		$this->assertEquals("SELECT * FROM `fairies` WHERE `fairies`.`id` MATCHES AGAINST ?  ", $query[0]);
        $this->assertEquals(array('phoenix'), $query[1]);
	}
	...
	{
		$stub = (object) array('db_type' => 'mysql');
		$subquery = new \PHPhoenix\DB\PDO\Query($stub, 'select');
	...
	public function testJoinSubtable()
	{
		...
		$this->object->join('phoenixs', array('fairies.id', '=', 'phoenix.id'));
		$this->object->join(array('fairies', 'fae'), array('fairies.id', '=', 'fae.id'));
		$this->object->join(array($subquery, 'fae2'), array('fairies.id', '=', 'fae2.id'));


		$this->assertEquals('SELECT * FROM (SELECT * FROM `fairies` )  AS `a0` LEFT JOIN `phoenixs` ON `fairies`.`id` = `phoenix`.`id` LEFT JOIN `fairies` AS `fae` ON `fairies`.`id` = `fae`.`id` LEFT JOIN (SELECT * FROM `fairies` )  AS `fae2` ON `fairies`.`id` = `fae2`.`id` ', current($this->object->query()));
	}

	public function testExpressionSelect()
	{
		$this->object->table('fairies')->where('id', 'in', new \PHPhoenix\DB\Expression("(SELECT id from fairies)"));
	...
	{
		$this->object->table('fairies')->where(new \PHPhoenix\DB\Expression('fairies.id'), 5);
	...
	public function testQueryUpdateWithJoins()
	{
		...
		$this->assertEquals('UPDATE `phoenixs` AS `p` LEFT JOIN `forest` AS `f` ON `p`.`forest_id` = `f`.`id` SET `p.name` = ? WHERE `f`.`name` = ?  ', current($this->object->query()));
	}
